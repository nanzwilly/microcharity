...
// Skip blocks that are specific to the old WordPress site and don't belong on the rebuild
// (UPI QR scan-and-pay block, bank transfer instructions, donor list widget headers).
function isCruftCaption(caption) {
  if (!caption) return false;
  const c = caption.toLowerCase();
  return c.includes('upi qr')
      || c.includes('scan & pay') || c.includes('scan and pay')
      || c.includes('bank transfer')
      || c.includes('donate via');
}

function parseUpdates(entryHtml) {
  // Format A — Special Text Boxes plugin (newer causes).
  //   <div class="… stb-container …"><span>caption</span>…<div class="stb-body-box">…</div></div>
  const stb = [];
  ...
  while ((m = containerRe.exec(entryHtml))) {
    ...
    const text = bodyM ? htmlToText(bodyM[1]) : '';
    if (isCruftCaption(caption)) continue;
    if (caption || text) stb.push({ caption, body: text });
  }
  if (stb.length) return stb;

  // Format B — older inline format: <h3><font color="red">date</font></h3> then paragraphs.
  // Scope to the give-form-content-wrap when present, else the whole entry-content tail.
  const wrapM = entryHtml.match(/<div[^>]*class=['"][^'"]*give[_-]form-content[^'"]*['"][^>]*>([\s\S]*?)(?:<\/article|<aside|<footer|<div[^>]*class=['"][^'"]*entry-meta|$)/i);
  const scope = wrapM ? wrapM[1] : entryHtml;

  // Split on <h3>…</h3> headings; everything between two h3s is the body for the preceding caption.
  const sections = [];
  const h3Re = /<h3[^>]*>([\s\S]*?)<\/h3>/gi;
  const captures = [];
  let h;
  while ((h = h3Re.exec(scope))) captures.push({ caption: stripTags(h[1]), end: h.index + h[0].length });
  for (let i = 0; i < captures.length; i++) {
    const start = captures[i].end;
    const stop = i + 1 < captures.length ? scope.indexOf('<h3', start) : scope.length;
    const bodyHtml = scope.slice(start, stop);
    const caption = captures[i].caption;
    const text = htmlToText(bodyHtml);
    if (isCruftCaption(caption)) continue;
    if (caption || text) sections.push({ caption, body: text });
  }
  if (sections.length) return sections;

  // Format C — fallback: nothing structured. Return the wrap content as a single un-captioned block.
  if (wrapM) {
    const text = htmlToText(wrapM[1]);
    if (text) return [{ caption: '', body: text }];
  }
  return [];
}
...
async function scrape(url) {
  ...
  // Prefer the data-amounts JSON attribute — it carries the canonical INR value.
  // Visible text on some pages is abbreviated (e.g. "₹4.1 lakh") and would parse wrong.
  const raisedDataM = html.match(/<span[^>]*class=['"][^'"]*income[^'"]*['"][^>]*data-amounts=['"]([^'"]+)['"][^>]*>/i);
  const goalDataM = html.match(/<span[^>]*class=['"][^'"]*goal-text[^'"]*['"][^>]*data-amounts=['"]([^'"]+)['"][^>]*>/i);
  const fromDataAmount = (raw) => {
    if (!raw) return null;
    // raw is HTML-encoded JSON like: { &quot;INR&quot;: &quot;410308&quot; } or { &quot;INR&quot;: 500000 }
    const m = decode(raw).match(/INR['"\s:]+['"]?(\d+)/i);
    return m ? parseInt(m[1], 10) : null;
  };
  let raised = fromDataAmount(raisedDataM?.[1]);
  let goal = fromDataAmount(goalDataM?.[1]);
  if (raised == null) {
    const m = html.match(/<span[^>]*class=['"][^'"]*income[^'"]*['"][^>]*>([\s\S]*?)<\/span>/i);
    raised = m ? parseAmount(stripTags(m[1])) : 0;
  }
  if (goal == null) {
    const m = html.match(/<span[^>]*class=['"][^'"]*goal-text[^'"]*['"][^>]*>([\s\S]*?)<\/span>/i);
    goal = m ? parseAmount(stripTags(m[1])) : 0;
  }
